...
def manhattanDistance(grid, goal, size, memo={}):
	...
	cost = (sum(abs(goal_pos[grid[index]] % size - index % size) + abs(goal_pos[grid[index]] // size - index // size)
		 for index in range(size**2) if grid[index] != 0))
...
def manhattanDistance(grid, goal, size, memo={}):
	...
	for index in range(size**2):
		...
		# col = index // size
		# goal_row, goal_col = goal_pos[grid[index]] % size, goal_pos[grid[index]] // size
		goal_index = goal_pos[grid[index]]
		cost += abs(index % size - goal_index % size) + abs(index // size - goal_index // size)
